      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <div className={`w-2 h-7 ${iconColor} rounded-full shadow-sm`}></div>
            <div>
              <h3 className="text-h-section leading-none">{title}</h3>
              {subtitle && <p className="text-label-caps mt-1.5">{subtitle}</p>}
            </div>
          </div>
        </div>
        {!isExpanded && (
          <span className="text-label-caps text-primary px-2">Click para expandir</span>
        )}
      </div>
